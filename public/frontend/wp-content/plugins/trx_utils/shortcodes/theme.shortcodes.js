// Init actions
function mr_murphy_sc_init_actions() {
	"use strict";
	setTimeout(function() {
		mr_murphy_sc_animation();
	}, 600);
	...
	jQuery('body')
		.on('click', '.show_popup_menuitem', function(e) {
			"use strict";
			mr_murphy_menuitems_show_popup(jQuery(this));
			e.preventDefault();
			return false;
		})
		.on('click', '.close_menuitem, .popup_menuitem', function(e) {
			...
			if (target.hasClass('popup_menuitem') || target.hasClass('close_menuitem') || target.parent().hasClass('close_menuitem')) {
				mr_murphy_menuitems_hide_popup();
				e.preventDefault();
				return false;
			}
		});
	...
	mr_murphy_sc_init(jQuery('body').eq(0));
}


// Resize actions
function mr_murphy_sc_resize_actions() {
	"use strict";
	mr_murphy_sc_sliders_resize();
	mr_murphy_sc_equal_height();
}


// Scroll actions
function mr_murphy_sc_scroll_actions() {
	"use strict";
	mr_murphy_sc_animation();
}
...
// Animation
function mr_murphy_sc_animation() {
...
// Shortcodes init
function mr_murphy_sc_init(container) {
...
// Scrolled areas
function mr_murphy_sc_init_scroll_area(obj) {
	"use strict";

	// Wait for images loading
	if (!mr_murphy_check_images_complete(obj) && MR_MURPHY_STORAGE['scroll_init_counter']++ < 30) {
		setTimeout(function() { mr_murphy_sc_init_scroll_area(obj); }, 200);
		return;
	}
...
// Slider navigation
function mr_murphy_sc_prepare_slider_navi(slider) {
	"use strict";
	var navi = null;
	...
	if (false) {
		...
		if (navi.length > 0) {
			navi.on('click', '.sc_slider_prev,.sc_scroll_prev', function(e) {
				...
				MR_MURPHY_STORAGE['swipers'][id].slidePrev();
				e.preventDefault();
				return false;
			})
			.on('click', '.sc_slider_next,.sc_scroll_next', function(e){
				...
				if (swiper.length == 0) swiper = jQuery(this).parents('.sc_slider_controls_wrap,.sc_scroll_controls_wrap').siblings('.swiper-slider-container');
				var id = swiper.attr('id');
				MR_MURPHY_STORAGE['swipers'][id].slideNext();
				e.preventDefault();
				return false;
			});
		}
	...
	if (navi.length > 0) {
		navi.on('click', '.post_item', function(e){
			...
			var id = swiper.attr('id');
			MR_MURPHY_STORAGE['swipers'][id].slideTo(jQuery(this).index()+1);
			e.preventDefault();
			return false;
		});
	}
...
// Sliders: Pagination
function mr_murphy_sc_change_active_pagination_in_slider(slider, idx) {
	"use strict";
...
// Sliders: Autoheight
function mr_murphy_sc_slider_autoheight(slider) {
	"use strict";
...
// Sliders: Resize
function mr_murphy_sc_sliders_resize() {
	"use strict";
	...
	jQuery(slider).each(function() {
		...
		if (last_width==0 || last_width!=width) {
			...
			if (MR_MURPHY_STORAGE['swipers'][id].params.slidesPerView != spv) {
				MR_MURPHY_STORAGE['swipers'][id].params.slidesPerView = spv;
				MR_MURPHY_STORAGE['swipers'][id].params.loopedSlides = spv;
				//MR_MURPHY_STORAGE['swipers'][id].reInit();
			}
			MR_MURPHY_STORAGE['swipers'][id].onResize();
		}
...
function mr_murphy_sc_equal_height() {
	"use strict";
...
// Skills init
function mr_murphy_sc_init_skills(container) {
	"use strict";
	...
	container.find('.sc_skills_item:not(.inited)').each(function () {
		...
		if (scrollPosition > scrollSkills) {
			...
			if (type == 'bar') {
				...
				mr_murphy_sc_animate_skills_counter(start, stop, speed-(dir!='unknown' ? 5 : 0), step, ed, total);
			} else if (type == 'counter') {
				mr_murphy_sc_animate_skills_counter(start, stop, speed - 5, step, ed, total);
			} else if (type == 'pie') {
				...
				if (total.length == 1) {
					mr_murphy_sc_animate_skills_counter(start, stop, Math.round(1500/steps), step, ed, total);
					pieData.push({
...
function mr_murphy_sc_animate_skills_counter(start, stop, speed, step, ed, total) {
	"use strict";
	start = Math.min(stop, start + step);
	total.text(start+ed);
	if (start < stop) {
		setTimeout(function () {
			mr_murphy_sc_animate_skills_counter(start, stop, speed, step, ed, total);
		}, speed);
	}
}

// Skills arc init
function mr_murphy_sc_init_skills_arc(container) {
	"use strict";
	if (arguments.length==0) var container = jQuery('body');
	container.find('.sc_skills_arc:not(.inited)').each(function () {
		...
		var o = {
			...
			diagram: function(){
				...
				r.circle(c, c, Math.round(w/2)).attr({ stroke: 'none', fill: MR_MURPHY_STORAGE['theme_bg_color'] ? MR_MURPHY_STORAGE['theme_bg_color'] : '#ffffff' });
				
				var title = r.text(c, c, arc.data('caption')).attr({
					font: Math.round(rad*0.75)+'px "'+MR_MURPHY_STORAGE['theme_font']+'"',
					fill: MR_MURPHY_STORAGE['theme_color'] ? MR_MURPHY_STORAGE['theme_color'] : '#909090'
				}).toFront();
...
// Countdown update
function mr_murphy_countdown(dt) {
	"use strict";
...
function mr_murphy_sc_form_validate(form){
	"use strict";
	...
	if (!form_custom) {
		...
		if (form.find('[name="username"]').length > 0) {
			rule = {
				field: "username",
				max_length: { value: 60, message: MR_MURPHY_STORAGE['strings']['name_long'] }
			};
			if (form.find('[name="username"][aria-required="true"]').length > 0)
				rule['min_length'] = { value: 1, message: MR_MURPHY_STORAGE['strings']['name_empty'] };
			rules.push(rule);
		}
		...
		if (form.find('[name="email"]').length > 0) {
			rule = {
				field: "email",
				max_length: { value: 60, message: MR_MURPHY_STORAGE['strings']['email_long'] },
				mask: { value: MR_MURPHY_STORAGE['email_mask'], message: MR_MURPHY_STORAGE['strings']['email_not_valid'] }
			};
			if (form.find('[name="email"][aria-required="true"]').length > 0)
				rule['min_length'] = { value: 7, message: MR_MURPHY_STORAGE['strings']['email_empty'] };
			rules.push(rule);
		}
		...
		if (form.find('[name="subject"]').length > 0) {
			rule = {
				field: "subject",
				max_length: { value: 100, message: MR_MURPHY_STORAGE['strings']['subject_long'] }
			};
			if (form.find('[name="subject"][aria-required="true"]').length > 0)
				rule['min_length'] = { value: 1, message: MR_MURPHY_STORAGE['strings']['subject_empty'] };
			rules.push(rule);
		}
		...
		if (form.find('[name="message"]').length > 0) {
			rule = {
				field: "message",
				max_length: { value: MR_MURPHY_STORAGE['contacts_maxlength'], message: MR_MURPHY_STORAGE['strings']['text_long'] }
			};
			if (form.find('[name="message"][aria-required="true"]').length > 0)
				rule['min_length'] = { value: 1, message: MR_MURPHY_STORAGE['strings']['text_empty'] };
			rules.push(rule);
		}

		error = mr_murphy_form_validate(form, {
	...
	if (!error && url!='#') {
		jQuery.post(url, {
			...
			if (rez.error === '') {
				form.get(0).reset();
				result.addClass("sc_infobox_style_success").html(MR_MURPHY_STORAGE['strings']['send_complete']);
				...
				result.addClass("sc_infobox_style_error").html(MR_MURPHY_STORAGE['strings']['send_error'] + ' ' + rez.error);
			}
...
function mr_murphy_select_players_category(sel) {
...
function mr_murphy_menuitems_show_popup(obj) {
	"use strict";
	
	// First init vars
	if (typeof MR_MURPHY_STORAGE['menuitem_load'] == 'undefined') {
		MR_MURPHY_STORAGE['menuitem_load'] = false;
		MR_MURPHY_STORAGE['menuitems_list'] = [];

	// If busy - return
	} else if ( MR_MURPHY_STORAGE['menuitem_load'] ) {
		return;
	}
	...
	if (MR_MURPHY_STORAGE['menuitems_list'].length == 0) {
		var menuitems_list_id = obj.parents('.sc_menuitems').attr("id");
		MR_MURPHY_STORAGE['menuitems_list'] = MR_MURPHY_STORAGE['menuitems'][menuitems_list_id].split(",");
	}
	
	var menuitem_id = obj.attr('rel');
	
	MR_MURPHY_STORAGE['menuitem_load'] = true;
	...
	jQuery.post(MR_MURPHY_STORAGE['ajax_url'], {
		...
		try {
			...
			rez = { error: MR_MURPHY_STORAGE['ajax_error'] + '<br>' + response };
		}
		...
		if (rez.error === '') {
			...
			setTimeout(function() {
				...
				if ( MR_MURPHY_STORAGE['menuitems_list'].length > 1 ) {
					var idx = 0;
					for (var i=0; i<MR_MURPHY_STORAGE['menuitems_list'].length; i++ ) {
						if ( MR_MURPHY_STORAGE['menuitems_list'][i] === menuitem_id ) {
							idx = i;
							break;
						}
					}
					var prev = (idx - 1 + MR_MURPHY_STORAGE['menuitems_list'].length) % MR_MURPHY_STORAGE['menuitems_list'].length;
					var next = (idx + 1) % MR_MURPHY_STORAGE['menuitems_list'].length;
					jQuery('.popup_menuitem .sc_menuitems_wrap .sc_menuitem_content').append(
						  "<a class='prev_menuitem prevnext_menuitem show_popup_menuitem' rel='"+MR_MURPHY_STORAGE['menuitems_list'][prev]+"'  href='#'><span class='icon-left'></span></a>"
						+ "<a class='next_menuitem prevnext_menuitem show_popup_menuitem' rel='"+MR_MURPHY_STORAGE['menuitems_list'][next]+"'  href='#'><span class='icon-right'></span></a>"
					);
				}
...
function mr_murphy_menuitems_hide_popup() {
	"use strict";
